...
import { KelvinToDmxTool } from "@/components/tools/KelvinToDmxTool";
...
type KelvinToDmxPageProps = {
  params: Promise<{ locale: AppLocale }>;
};

export async function generateMetadata({ params }: KelvinToDmxPageProps): Promise<Metadata> {
  const { locale: localeParam } = await params;
  const locale = normalizeLocale(localeParam);
  const dictionary = await getDictionary(locale);
  const canonicalPath = prefixLocale(locale, "/tools/kelvin-to-dmx");
  const ogAlt = dictionary.meta?.ogAlt ?? "Y-Link";

  return {
    title: dictionary.tools.kelvinToDmx.metadata.title,
    description: dictionary.tools.kelvinToDmx.metadata.description,
    alternates: {
      canonical: canonicalPath,
      languages: {
        "nb-NO": prefixLocale("nb", "/tools/kelvin-to-dmx"),
        "en-US": prefixLocale("en", "/tools/kelvin-to-dmx"),
      },
    },
    openGraph: {
      title: dictionary.tools.kelvinToDmx.metadata.title,
...
export default async function KelvinToDmxPage({ params }: KelvinToDmxPageProps) {
  const { locale: localeParam } = await params;
  const locale = normalizeLocale(localeParam);
  const dictionary = await getDictionary(locale);
  const lang = getLanguageTag(locale);
  const { tools, navigation } = dictionary;
  const tool = tools.kelvinToDmx;
  ...
  return (
    <ToolPage
      ...
      canonicalPath={prefixLocale(locale, "/tools/kelvin-to-dmx")}
