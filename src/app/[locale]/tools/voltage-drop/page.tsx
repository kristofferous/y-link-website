...
type VoltageDropPageProps = {
  params: Promise<{ locale: AppLocale }>;
};

export async function generateMetadata({ params }: VoltageDropPageProps): Promise<Metadata> {
  const { locale: localeParam } = await params;
  const locale = normalizeLocale(localeParam);
  const dictionary = await getDictionary(locale);
  const canonicalPath = prefixLocale(locale, "/tools/voltage-drop");
  const ogAlt = dictionary.meta?.ogAlt ?? "Y-Link";

  return {
    title: dictionary.tools.voltageDrop.metadata.title,
    description: dictionary.tools.voltageDrop.metadata.description,
    alternates: {
      canonical: canonicalPath,
      languages: {
        "nb-NO": prefixLocale("nb", "/tools/voltage-drop"),
        "en-US": prefixLocale("en", "/tools/voltage-drop"),
      },
    },
    openGraph: {
      title: dictionary.tools.voltageDrop.metadata.title,
...
export default async function VoltageDropPage({ params }: VoltageDropPageProps) {
  const { locale: localeParam } = await params;
  const locale = normalizeLocale(localeParam);
  const dictionary = await getDictionary(locale);
  const lang = getLanguageTag(locale);
  const { tools, navigation } = dictionary;
  const tool = tools.voltageDrop;
  ...
  return (
    <ToolPage
      ...
      canonicalPath={prefixLocale(locale, "/tools/voltage-drop")}
