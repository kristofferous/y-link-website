...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${cluster.tag} Cluster | Y-Link`,
    description: `Curated ${cluster.tag} guides and blog posts.`,
    alternates: {
      canonical: prefixLocale(locale, `/clusters/${cluster.slug}`),
      languages: {
        "nb-NO": prefixLocale("nb", `/clusters/${cluster.slug}`),
        "en-US": prefixLocale("en", `/clusters/${cluster.slug}`),
      },
    },
  };
}
        ...
        <div className="container-custom space-y-8">
          <Breadcrumbs
            items={[
              ...
              { label: cluster.tag, href: prefixLocale(locale, `/clusters/${cluster.slug}`) },
            ]}
          />

          <div className="space-y-4">
            <p className="text-label text-muted-foreground">Content Cluster</p>
